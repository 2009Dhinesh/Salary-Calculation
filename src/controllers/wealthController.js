...
const getWealthDashboard = async (req, res, next) => {
  try {
    ...
    const cacheKey = `wealth:${userId}`;

    // Check Cache
    const cachedData = await cache.get(cacheKey);
    if (cachedData) {
      return res.json({ success: true, dashboard: cachedData, fromCache: true });
    }

    // Fetch static assets in parallel
    const [accounts, metalAssets, landAssets, debtsGiven, debtsBorrowed] = await Promise.all([
      Account.find({ user: userId, isArchived: { $ne: true } })
        .select('name balance icon color bankName bankLogo isArchived')
        .lean(),
    ...
    await cache.set(cacheKey, dashboardData, 3600);
